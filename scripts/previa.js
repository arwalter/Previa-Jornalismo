...
function deleteInterval(interval) {
    document.getElementById('interval').childNodes.forEach((value)=>{value.remove()});
    intervalCount--;
    fillIntervals();
}
...
function fillIntervals(){
    ...
    <div class="input-group input-group-sm mb-3">
        ...
        <input id="timeIn${index+1}"  type="time" class="form-control interval">
        <input id="timeOut${index+1}" type="time" class="form-control interval">
        <div class="input-group-append">
            <button type="button" class="btn btn-danger" id="trash${index+1}" onClick="deleteInterval(${index+1})">
